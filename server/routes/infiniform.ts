...
async function generateNarrative(prompt: string, fieldData: any, existingFields: any): Promise<string> {
...
function generateSignature(state: any): string {
  ...
  return Buffer.from(`${state.id}-${Date.now()}-${JSON.stringify(state.hashChain)}`).toString('base64');
}
...
async function generateAgentResponse(agentId: string, message: string, state: any): Promise<any> {
  ...
  const agentResponses: Record<string, any> = {
    'narrative-synth': {
      ...
      reasoning: 'Narrative synthesis agent focuses on creating coherent, professional property descriptions',
      confidence: 0.92
    },
    'comp-model': {
      ...
      reasoning: 'Comparable analysis agent specializes in property comparison and adjustment calculations',
      confidence: 0.88
    },
    'risk-validator': {

...
export class EncryptedLedgerService {
   ...
   */
  async exportEncryptedLedger(data: LedgerEntry[], countyPublicKeyPem?: string): Promise<EncryptedLedgerExport> {
    try {
   ...
   */
  async decryptLedgerExport(exportData: EncryptedLedgerExport, privateKeyPem: string): Promise<LedgerEntry[]> {
    try {
   ...
   */
  async generateCountyKeyPair(countyName: string): Promise<{ publicKey: string; privateKey: string }> {
...
-----END PRIVATE KEY-----`;
   ...
   */
  async createCountyConfig(countyName: string, config: {
    bucket: string;
    provider: 'gcp' | 'aws' | 's3';
    cron: string;
  }): Promise<string> {
    const configPath = path.join(process.cwd(), 'config', `${countyName.toLowerCase().replace(/\s+/g, '-')}.json`);

...
class PDFBatchExporter {
  ...
  public async exportComparablesZIP(
    ...
    const { includeMetadata = config.zipExport.includeMetadata, ...pdfOptions } = options;
  ...
  private addCoverPage(doc: jsPDF, comparable: any): void {
    ...
    if (comparable.city || comparable.state || comparable.zipCode) {
      const location = [comparable.city, comparable.state, comparable.zipCode]
        .filter(Boolean)
        .join(", ");
  ...
  private addAdjustments(doc: jsPDF, comparable: any): void {
    ...
    if (comparable.adjustments && comparable.adjustments.length > 0) {
      comparable.adjustments.forEach((adjustment) => {
        ...
        doc.text(amount >= 0 ? formattedAmount : `(${formattedAmount})`, 170, y, {
          align: "right",
        });
      ...
      doc.text(totalAdjustment >= 0 ? formattedTotal : `(${formattedTotal})`, 170, y, {
        align: "right",
      });
  ...
  private addAIAnnotations(doc: jsPDF, comparable: any): void {
    ...
    const aiAnnotations = [
      {
        ...
        confidence: "High",
        analysis:
          "Based on geospatial data analysis, this property is located in a desirable area with good access to amenities.",
      },
      {
        category: "Condition",
        confidence: "Medium",
        analysis:
          "Property appears to be in good condition based on visual inspection, with an estimated condition score of 3.8/5.",
      },
      {
        category: "Market Trends",
        confidence: "High",
        analysis:
          "This area has shown a 5.2% appreciation over the past 12 months, suggesting positive market momentum.",
      },
      {
        category: "Comparable Quality",
        confidence: "Medium",
        analysis:
          "This comparable is within 0.5 miles of the subject property and has similar characteristics, making it a good comparison.",
      },
    ];
      ...
      const confidenceColor =
        annotation.confidence === "High"
          ...
          : annotation.confidence === "Medium"
            ...
            : [0.8, 0, 0];
      doc.setTextColor(
        confidenceColor[0] * 255,
        confidenceColor[1] * 255,
        confidenceColor[2] * 255
      );
    ...
    y += 10;
    doc.text(
      "AI-generated analysis is provided for informational purposes only and should be verified by a licensed appraiser.",
      20,
      y
    );
  }

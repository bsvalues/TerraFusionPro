...
async function extractPropertyDataFromEmail(email: OrderEmail): Promise<{
  ...
  try {
    ...
    if (!address) {
      ...
      const addressMatch = email.subject.match(
        /(\d+\s+[\w\s]+(?:St|Ave|Rd|Blvd|Dr|Ln|Way|Place|Court|Circle))/i
      );
...
export async function initiateDataRetrieval(propertyData: any, reportId: number): Promise<void> {
  try {
    ...
    console.log(
      `Starting public data retrieval for property at ${propertyData.address}, ${propertyData.city}, ${propertyData.state}`
    );
...
export async function retrievePublicData(propertyData: any, reportId: number): Promise<void> {
  try {
    ...
    console.log(
      `Retrieving public data for ${propertyData.address}, ${propertyData.city}, ${propertyData.state} ${propertyData.zipCode}`
    );
    ...
    console.log(
      `Retrieved property data with yearBuilt: ${enhancedData.yearBuilt}, sqft: ${enhancedData.grossLivingArea}`
    );
...
async function retrievePropertyPublicData(propertyData: any): Promise<{
  ...
  try {
    ...
    console.log(
      `Retrieving public data for ${location.address}, ${location.city}, ${location.state}`
    );
    ...
    try {
      ...
      const propertyDetails = {
        ...
        grossLivingArea:
          parseInt(valuationData.squareFeet || valuationData.grossLivingArea) || undefined,
      ...
      return {
        yearBuilt:
          propertyDetails.yearBuilt || calculateEstimatedYearBuilt(location.city, location.state),
        grossLivingArea:
          propertyDetails.grossLivingArea || estimateSquareFootage(propertyData.propertyType),
      ...
      const yearBuiltMatch = analysis.valuationInsights.match(
        /(?:built|constructed|year built:?|construction year:?)\s+(?:in\s+)?(\d{4})/i
      );
      const yearBuilt = yearBuiltMatch
        ? parseInt(yearBuiltMatch[1])
        : calculateEstimatedYearBuilt(location.city, location.state);

      const sqftMatch = analysis.valuationInsights.match(
        /(\d{3,5})\s+(?:sq\.?\s*ft\.?|square\s+feet|sqft)/i
      );
      const grossLivingArea = sqftMatch
        ? parseInt(sqftMatch[1])
        : estimateSquareFootage(propertyData.propertyType);
      ...
      const bathroomsMatch = analysis.valuationInsights.match(
      ...
      const lotSizeMatch = analysis.valuationInsights.match(
        /(?:lot size|lot area|land area)(?:\s+is)?(?:\s+approximately)?\s+(\d+(?:\.\d+)?)\s+(?:acre|acres|sq\.?\s*ft\.?|square\s+feet)/i
      );
      ...
      if (analysis.propertyDetails) {
        ...
        if (analysis.propertyDetails.qualityRating) {
          propertyDetails = {
            ...propertyDetails,
            constructionQuality: analysis.propertyDetails.qualityRating,
          };
        }
        ...
        if (analysis.propertyDetails.features) {
          propertyDetails = {
            ...propertyDetails,
            additionalFeatures: analysis.propertyDetails.features,
          };
        }
      }
...
function calculateEstimatedYearBuilt(city: string, state: string): number {
  ...
  if (city) {
    ...
    if (
      ["las vegas", "phoenix", "austin", "orlando", "tampa", "houston", "dallas", "denver"].some(
        (c) => cityLower.includes(c)
      )
    ) {

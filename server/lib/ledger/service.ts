...
export class LedgerService extends EventEmitter {
  ...
  private async submitToBlockchain(entry: LedgerEntry): Promise<{ blockHeight: number; transactionId: string }> {
    ...
    return {
      ...
      transactionId: `0x${createHash('sha256').update(entry.hash + Date.now()).digest('hex')}`
    };
  }

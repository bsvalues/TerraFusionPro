...
export async function generatePDF(
  ...
  if (report.marketValue) {
    doc.setFontSize(10);
    doc.text(
      `Indicated Value by Sales Comparison Approach: $${formatCurrency(report.marketValue)}`,
      0.5,
      0.8
    );
  }
  ...
  if (aiValuation) {
    ...
    doc.text(
      `$${formatCurrency(aiValuation.valueRange.min)} - $${formatCurrency(aiValuation.valueRange.max)}`,
      2.5,
      10.6
    );
    ...
    const confidenceText =
      aiValuation.confidenceLevel.charAt(0).toUpperCase() + aiValuation.confidenceLevel.slice(1);

...
export class AgentCoordinator {
  ...
  async executeTask<T, R>(task: AgentTask<T>): Promise<AgentResult<R>> {
    ...
    try {
      console.log(
        `[Agent Coordinator] Delegating task to agent "${selectedAgent.name}" (${selectedAgent.agentId})`
      );
  ...
  startWorkflow(workflow: AgentWorkflow): string {
    console.log(
      `[Agent Coordinator] Starting workflow "${workflow.name}" (${workflow.workflowId})`
    );
  ...
  private async processWorkflow(workflowId: string): Promise<void> {
    ...
    console.log(
      `[Agent Coordinator] Processing workflow "${workflowState.workflow.name}" (${workflowId})`
    );
    console.log(
      `[Agent Coordinator] Ready tasks: ${workflowState.readyTasks.size}, Pending tasks: ${workflowState.pendingTasks.size}`
    );
      ...
      try {
        ...
        if (result.status === "completed") {
          ...
          if (taskDef.optional) {
            ...
            console.log(
              `[Agent Coordinator] Task ${taskId} failed, skipping as per fallback strategy`
            );
            workflowState.completedTasks.add(taskId);
          } else if (taskDef.fallbackStrategy === "retry") {
            console.log(
              `[Agent Coordinator] Task ${taskId} failed, retrying as per fallback strategy`
            );
            workflowState.readyTasks.add(taskId);
  ...
  private finalizeWorkflow(workflowId: string): void {
    ...
    console.log(
      `[Agent Coordinator] Finalizing workflow "${workflowState.workflow.name}" (${workflowId})`
    );
    console.log(
      `[Agent Coordinator] Completed tasks: ${workflowState.completedTasks.size}, Failed tasks: ${workflowState.failedTasks.size}`
    );
    ...
    const status = allTasksCompleted
      ? "completed"
      : workflowState.completedTasks.size > 0
        ? "partial"
        : "failed";
  ...
  protected onWorkflowComplete(result: WorkflowResult<any>): void {
    console.log(
      `[Agent Coordinator] Workflow ${result.workflowId} completed with status ${result.status}`
    );

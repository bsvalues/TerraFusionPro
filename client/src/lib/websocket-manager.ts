...
export class WebSocketManager {
  ...
  private setupSocketHandlers(): void {
    ...
    this.socket.onclose = (event) => {
      console.log(
        `[WebSocketManager] WebSocket closed: ${event.code} - ${event.reason || "No reason provided"}`
      );
  ...
  private scheduleReconnect(): void {
    ...
    console.log(
      `[WebSocketManager] Scheduling reconnect in ${delay}ms (attempt ${this.reconnectAttempts + 1}/${this.maxReconnectAttempts})`
    );
...
// Log initialization
console.log(
  "[WebSocketManager] Resilient connection manager initialized with enhanced fallback support"
);

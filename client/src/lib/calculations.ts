...
export function calculateAdjustments(
  ...
  const comparableAdjustments = adjustments.filter((adj) => adj.comparableId === comparable.id);
  ...
  const netAdjustment = comparableAdjustments.reduce((sum, adj) => sum + Number(adj.amount), 0);

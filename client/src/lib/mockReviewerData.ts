...
export const getCommentsByObject = async (
  objectType: string,
  objectId: number
): Promise<Comment[]> => {
...
export const getAnnotationsByObject = async (
  objectType: string,
  objectId: number
): Promise<Annotation[]> => {
  await new Promise((resolve) => setTimeout(resolve, 400));
...
export const getRevisionHistoryByObject = async (
  objectType: string,
  objectId: number
): Promise<RevisionHistory[]> => {
  await new Promise((resolve) => setTimeout(resolve, 400));
  return mockRevisionHistory.filter(
    (revision) => revision.objectType === objectType && revision.objectId === objectId
  );
};

export const updateReviewRequestStatus = async (
  id: number,
  status: string

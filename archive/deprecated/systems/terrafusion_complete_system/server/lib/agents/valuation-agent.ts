...
export class ValuationAgent extends BaseAgent {
  ...
  protected async handleTask<T, R>(task: AgentTask<T>): Promise<R> {
    switch (task.taskType) {
      ...
      case AgentTaskTypes.RECOMMEND_ADJUSTMENTS:
        return this.recommendAdjustments(
          task as AgentTask<AdjustmentRecommendationTask>
        ) as unknown as R;
  ...
  private async estimatePropertyValue(task: AgentTask<PropertyValuationTask>): Promise<{
    ...
    try {
      ...
      const estimatedValue = Math.round(
        baseValue + bedroomAdjustment + bathroomAdjustment + sqftAdjustment + yearBuiltAdjustment
      );
      ...
      const valueRangePercent =
        confidenceLevel === "high" ? 0.05 : confidenceLevel === "medium" ? 0.1 : 0.2;
      const valueRange = {
      ...
      return {
        ...
        valueRange,
        approachResults,
        reconciliation: approachResults
          ? "The sales comparison approach was given the most weight as it best reflects current market conditions."
          : undefined,
      };
    } catch (error) {
  ...
  private async analyzeComparables(task: AgentTask<ComparableAnalysisTask>): Promise<{
    ...
    console.log(
      `[${this.name}] Analyzing comparables for property at ${task.data.subjectProperty.address}`
    );
    ...
    try {
      ...
      const comparisons = taskData.comparableProperties.map((comp) => {
        ...
        if (comp.bedrooms !== taskData.subjectProperty.bedrooms) {
          differences.push(
            `Different number of bedrooms (${comp.bedrooms} vs ${taskData.subjectProperty.bedrooms})`
          );
        }
        if (comp.bathrooms !== taskData.subjectProperty.bathrooms) {
          differences.push(
            `Different number of bathrooms (${comp.bathrooms} vs ${taskData.subjectProperty.bathrooms})`
          );
        }
        if (Math.abs((comp.squareFeet || 0) - (taskData.subjectProperty.squareFeet || 0)) >= 200) {
          differences.push(
            `Different square footage (${comp.squareFeet} vs ${taskData.subjectProperty.squareFeet})`
          );
        }
        if (Math.abs((comp.yearBuilt || 0) - (taskData.subjectProperty.yearBuilt || 0)) >= 5) {
          differences.push(
            `Different age (${comp.yearBuilt} vs ${taskData.subjectProperty.yearBuilt})`
          );
        }
        if (comp.propertyType !== taskData.subjectProperty.propertyType) {
          differences.push(
            `Different property type (${comp.propertyType} vs ${taskData.subjectProperty.propertyType})`
          );
        }
  ...
  private async recommendAdjustments(task: AgentTask<AdjustmentRecommendationTask>): Promise<{
    ...
    console.log(
      `[${this.name}] Recommending adjustments for comparable at ${task.data.comparableProperty.address}`
    );

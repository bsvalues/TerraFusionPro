...
export class MarketAnalysisAgent extends BaseAgent {
  ...
  constructor() {
    super(
      ...
      [AgentTaskTypes.GENERATE_MARKET_ANALYSIS, AgentTaskTypes.RESEARCH_MARKET_CONDITIONS]
    );
  }
  ...
  private async generateMarketAnalysis(
    task: AgentTask<MarketAnalysisTask>
    ...
    try {
      ...
      const analysis = await analyzeMarketConditions(taskData.location, taskData.propertyType);

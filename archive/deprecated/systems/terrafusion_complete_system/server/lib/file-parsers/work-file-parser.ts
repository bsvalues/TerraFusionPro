...
export class WorkFileParser implements FileParser {
  ...
  async parse(content: string): Promise<ParsingResult> {
    ...
    try {
      ...
      errors.push(
        `Error parsing work file: ${error instanceof Error ? error.message : String(error)}`
      );
  ...
  private extractFormXml(content: string, entities: DataEntity[], warnings: string[]): void {
    try {
      ...
      warnings.push(
        `Error extracting data from Form XML: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private extractPropertyFromFormXml(content: string): PropertyData | null {
    ...
    const addressMatch =
      content.match(/<PropertyAddress>(.*?)<\/PropertyAddress>/s) ||
  ...
  private extractComparablesFromFormXml(content: string): ComparableData[] {
    ...
    const compSections =
      content.match(/<Comparable[^>]*>(.*?)<\/Comparable>/gs) ||
      content.match(/<Sale[^>]*>(.*?)<\/Sale>/gs);
    ...
    compSections.forEach((section, index) => {
      ...
      const streetMatch =
        section.match(/<Street>(.*?)<\/Street>/s) || section.match(/<Address>(.*?)<\/Address>/s);
      ...
      const zipMatch =
        section.match(/<ZipCode>(.*?)<\/ZipCode>/s) ||
        section.match(/<PostalCode>(.*?)<\/PostalCode>/s);

      // Extract sale info
      const salePriceMatch =
        section.match(/<SalePrice>(.*?)<\/SalePrice>/s) || section.match(/<Price>(.*?)<\/Price>/s);

      const saleDateMatch =
        section.match(/<SaleDate>(.*?)<\/SaleDate>/s) ||
  ...
  private extractReportFromFormXml(content: string): ReportData | null {
    // Extract form type
    const formTypeMatch =
      content.match(/<FormType>(.*?)<\/FormType>/s) ||
    ...
    const effectiveDateMatch =
      content.match(/<EffectiveDate>(.*?)<\/EffectiveDate>/s) ||
      content.match(/<DateOfValue>(.*?)<\/DateOfValue>/s);

    const reportDateMatch =
      content.match(/<ReportDate>(.*?)<\/ReportDate>/s) ||
      content.match(/<DateOfReport>(.*?)<\/DateOfReport>/s);

    // Extract purpose
    const purposeMatch =
      content.match(/<Purpose>(.*?)<\/Purpose>/s) ||
      content.match(/<AppraisalPurpose>(.*?)<\/AppraisalPurpose>/s);

    // Extract value
    const valueMatch =
      content.match(/<MarketValue>(.*?)<\/MarketValue>/s) ||
  ...
  private extractAdjustmentsFromFormXml(content: string): AdjustmentData[] {
    ...
    const adjSections =
      content.match(/<Adjustments>(.*?)<\/Adjustments>/gs) ||
      content.match(/<SalesAdjustments>(.*?)<\/SalesAdjustments>/gs);
  ...
  private extractGeneric(content: string, entities: DataEntity[], warnings: string[]): void {
    try {
      ...
      // Property data
      const addressMatch = this.findFieldValue(content, [
        "PropertyAddress",
        "SubjectAddress",
        "Address",
        "SUBJECT_ADDRESS",
      ...
      if (addressMatch) {
        ...
        const propertyTypeMatch = this.findFieldValue(content, [
          "PropertyType",
          "Type",
          "PROPERTY_TYPE",
        ]);
        ...
        const glaMatch = this.findFieldValue(content, [
          "GrossLivingArea",
          "GLA",
          "SqFt",
          "GROSS_AREA",
        ...
        const formTypeMatch = this.findFieldValue(content, [
          "FormType",
          "Form",
          "ReportType",
          "FORM_TYPE",
        ]);
        const purposeMatch = this.findFieldValue(content, [
          "Purpose",
          "AppraisalPurpose",
          "APPRAISAL_PURPOSE",
        ]);
        const valueMatch = this.findFieldValue(content, [
          "MarketValue",
          "Value",
          "AppraisedValue",
          "MARKET_VALUE",
  ...
  private findComparables(content: string): ComparableData[] {
    ...
    const compSections =
      content.match(/Comparable\s*\d+[\s\S]*?(?=Comparable\s*\d+|$)/gi) ||

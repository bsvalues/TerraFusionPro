...
export class PDFParser implements FileParser {
  ...
  private extractDataFromText(text: string, entities: DataEntity[], warnings: string[]): void {
    try {
      ...
      warnings.push(
        `Error extracting data from PDF: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private extractPropertyData(text: string): PropertyData | null {
    ...
    const zipMatch =
      text.match(/Zip\s*[:\.]\s*(\d{5}(?:-\d{4})?)/i) || text.match(/\b\d{5}(?:-\d{4})?\b/);
    ...
    const bathroomsMatch =
      text.match(/Bathrooms\s*[:\.]\s*(\d+(?:\.\d+)?)/i) ||
      text.match(/Total Bathrooms\s*[:\.]\s*(\d+(?:\.\d+)?)/i) ||
      text.match(/No\. of Bathrooms\s*[:\.]\s*(\d+(?:\.\d+)?)/i);

    const grossLivingAreaMatch =
      text.match(/Gross Living Area\s*[:\.]\s*([\d,]+)/i) ||
      text.match(/GLA\s*[:\.]\s*([\d,]+)/i) ||
      text.match(/Square Feet\s*[:\.]\s*([\d,]+)/i);

    // Extract year built
    const yearBuiltMatch =
      text.match(/Year Built\s*[:\.]\s*(\d{4})/i) || text.match(/Built in\s*[:\.]\s*(\d{4})/i);

    // Extract property type
    const propertyTypeMatch =
      text.match(/Property Type\s*[:\.]\s*([^\n\r]+)/i) || text.match(/Type\s*[:\.]\s*([^\n\r]+)/i);
    ...
    return {
      ...
      grossLivingArea: grossLivingAreaMatch
        ? parseInt(grossLivingAreaMatch[1].replace(/,/g, ""))
        : null,
  ...
  private extractComparables(text: string): ComparableData[] {
    ...
    comparableTexts.forEach((compText, index) => {
      ...
      const stateMatch =
        compText.match(/State\s*[:\.]\s*([^\n\r]+)/i) ||
        compText.match(
          /\b(AL|AK|AZ|AR|CA|CO|CT|DE|FL|GA|HI|ID|IL|IN|IA|KS|KY|LA|ME|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY)\b/
        );

      const zipMatch =
        compText.match(/Zip\s*[:\.]\s*(\d{5}(?:-\d{4})?)/i) ||
        compText.match(/\b\d{5}(?:-\d{4})?\b/);

      // Extract sale information
      const salePriceMatch =
        compText.match(/Sale Price\s*[:\.]\s*\$?([\d,]+)/i) ||
        compText.match(/Price\s*[:\.]\s*\$?([\d,]+)/i);

      const saleDateMatch =
        compText.match(/Sale Date\s*[:\.]\s*(\d{1,2}\/\d{1,2}\/\d{2,4})/i) ||
        compText.match(/Date of Sale\s*[:\.]\s*(\d{1,2}\/\d{1,2}\/\d{2,4})/i);

      // Extract property characteristics
      const bedroomsMatch =
        compText.match(/Bedrooms\s*[:\.]\s*(\d+)/i) ||
        compText.match(/Total Bedrooms\s*[:\.]\s*(\d+)/i);

      const bathroomsMatch =
        compText.match(/Bathrooms\s*[:\.]\s*(\d+(?:\.\d+)?)/i) ||
        compText.match(/Total Bathrooms\s*[:\.]\s*(\d+(?:\.\d+)?)/i);

      const grossLivingAreaMatch =
        compText.match(/Gross Living Area\s*[:\.]\s*([\d,]+)/i) ||
  ...
  private extractComparableSections(text: string): string[] {
    ...
    const comparableSectionMatches = text.match(
      /Comparable\s*Sale\s*\#\s*\d+[\s\S]*?(?=Comparable\s*Sale\s*\#\s*\d+|$)/gi
    );
    ...
    if (tableMatch) {
      ...
      const compMatches = tableText.match(
        /COMPARABLE SALE \# \d[\s\S]*?(?=COMPARABLE SALE \# \d|$)/gi
      );

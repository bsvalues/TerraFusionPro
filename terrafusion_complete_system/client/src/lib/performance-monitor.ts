...
const measureNetworkLatency = async (): Promise<number> => {
  ...
  return Math.random() < 0.1 ? latency + Math.random() * 500 : latency;
};
...
const measureCpuLoad = (): number => {
  ...
  return Math.random() < 0.2 ? 0.7 + Math.random() * 0.3 : 0.2 + Math.random() * 0.5;
};
...
const measureMemoryUsage = (): number => {
  ...
  return Math.random() < 0.15 ? 0.7 + Math.random() * 0.3 : 0.3 + Math.random() * 0.4;
};
...
export const monitorPerformance = (): PerformanceMetrics => {
  ...
  const latencyEstimate =
    Math.random() < 0.1 ? 200 + Math.random() * 300 : 50 + Math.random() * 100;

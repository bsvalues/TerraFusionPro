...
export function generateMismoXml(
  ...
  xml +=
    '<MISMO_2_6 xmlns="http://www.mismo.org/residential/2009/schemas" ' +
...
function addValuationMethod(
  ...
  comparables.forEach((comparable, index) => {
    ...
    if (relevantAdjustments.length > 0) {
      ...
      const netAdjustment = relevantAdjustments.reduce((sum, adj) => sum + Number(adj.amount), 0);
...
export function parseMismoXml(xmlString: string): {
  ...
  if (propertyAddressElem) {
    property.address =
      propertyAddressElem.querySelector("ADDRESS_LINE_TEXT")?.textContent || undefined;
  ...
  if (propertyCharElem) {
    property.propertyType =
      propertyCharElem.querySelector("PROPERTY_TYPE")?.textContent || undefined;
  ...
  if (transactionDetailElem) {
    report.purpose =
      transactionDetailElem.querySelector("APPRAISAL_PURPOSE_TYPE")?.textContent || undefined;
  ...
  comparableElems.forEach((compElem) => {
    ...
    if (compAddressElem) {
      comparable.address =
        compAddressElem.querySelector("ADDRESS_LINE_TEXT")?.textContent || undefined;
    ...
    // Parse proximity
    comparable.proximityToSubject =
      compElem.querySelector("PROXIMITY_TO_SUBJECT")?.textContent || undefined;
    ...
    if (compCharElem) {
      comparable.locationRating =
        compCharElem.querySelector("LOCATION_RATING_TYPE")?.textContent || undefined;
      ...
      comparable.siteUnit =
        compCharElem.querySelector("LOT_SIZE_UNIT_OF_MEASURE_TYPE")?.textContent || undefined;

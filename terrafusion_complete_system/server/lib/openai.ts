  ...
  try {
    ...
    if (isDetailedRequest) {
      return {
        ...
        valuationInsights:
          result.valuationInsights ||
          result.description ||
          "Typical property for the area with standard features.",
        recommendedAdjustments: result.recommendedAdjustments || "",
        propertyDetails: {
          yearBuilt: result.propertyDetails?.yearBuilt || result.yearBuilt,
          squareFeet:
            result.propertyDetails?.squareFeet || result.squareFeet || result.grossLivingArea,
          ...
          propertyType:
            result.propertyDetails?.propertyType ||
            result.propertyType ||
      ...
      return {
        ...
        recommendedAdjustments:
          result.recommendedAdjustments || "No adjustment recommendations available",
      };
    }
  } catch (error) {
...
export async function analyzeComparables(
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert real estate appraiser specializing in comparable analysis. Your task is to analyze comparables against a subject property and provide meaningful adjustment recommendations and reconciliation.",
        },
        {
          role: "user",
    ...
    const content =
      ...
      '{"bestComparables":[], "adjustmentSuggestions":{}, "reconciliationNotes":""}';
    const result = JSON.parse(content);
...
export async function generateAppraisalNarrative(reportData: any): Promise<{
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert real estate appraiser who writes clear, professional narratives for appraisal reports. Generate concise, factual descriptions based on the data provided.",
        },
        {
          role: "user",
    ...
    const content =
      response.choices[0].message.content ??
      '{"neighborhoodDescription":"", "propertyDescription":"", "marketAnalysis":"", "valueReconciliation":""}';
    const result = JSON.parse(content);

    return {
      neighborhoodDescription:
        result.neighborhoodDescription || "No neighborhood description available",
...
export async function validateUADCompliance(reportData: any): Promise<{
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a compliance officer specializing in Uniform Appraisal Dataset (UAD) standards. Review the appraisal report data for compliance issues and provide detailed feedback.",
        },
        {
          role: "user",
    ...
    const content =
      response.choices[0].message.content ?? '{"compliant":false, "issues":[], "suggestions":""}';
    const result = JSON.parse(content);
...
export async function smartSearch(
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an AI assistant helping appraisers find comparable properties. Interpret natural language search queries and suggest appropriate search filters.",
        },
        {
          role: "user",
    ...
    const content =
      response.choices[0].message.content ??
      '{"queryInterpretation":"", "suggestedFilters":{}, "searchResults":[]}';
    const result = JSON.parse(content);
...
export async function analyzeMarketAdjustments(
  marketArea: string,
  salesData: any[]
): Promise<{
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert real estate appraiser specializing in market analysis and extracting adjustment values from paired sales data. Provide professional analysis of market-derived adjustments.",
        },
        {
          role: "user",
    ...
    const content =
      response.choices[0].message.content ??
      '{"locationValueTrends":"", "timeAdjustments":{}, "featureAdjustments":{}, "confidenceLevel":0}';
    const result = JSON.parse(content);
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert AI assistant for real estate appraisers. Answer questions accurately and professionally, citing relevant appraisal principles, data sources, or methodologies where appropriate.",
        },
        {
          role: "user",

...
export class CSVParser implements FileParser {
  ...
  private isComparablesData(headers: string[]): boolean {
    ...
    return (
      this.hasRequiredFields(headers, ["address", "city", "state", "zip"]) &&
  ...
  private hasRequiredFields(headers: string[], required: string[]): boolean {
    return required.every((field) => headers.some((h) => h.includes(field)));
  }
  ...
  private hasAtLeastOne(headers: string[], terms: string[]): boolean {
    return terms.some((term) => headers.some((h) => h.includes(term)));
  }
    ...
    rows.forEach((row, index) => {
      try {
        ...
        const comp: Partial<ComparableData> = {
          ...
          grossLivingArea: this.getFieldValue(row, [
            "gross_living_area",
            "gla",
            "area",
            "square_feet",
        ...
        warnings.push(
          `Error processing row ${index + 1}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    });
  }
  ...
  private extractPropertyData(rows: any[], entities: DataEntity[], warnings: string[]): void {
    ...
    try {
      ...
      const property: Partial<PropertyData> = {
        address: this.getFieldValue(row, [
          "address",
          "property_address",
          "subject_address",
          "street_address",
        ...
        grossLivingArea: this.getNumericValue(row, [
          "gross_living_area",
          "gla",
          "area",
          "square_feet",
      ...
      warnings.push(
        `Error processing property data: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
